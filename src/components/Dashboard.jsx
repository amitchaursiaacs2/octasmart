...
const Dashboard = () => {
  ...
  const connectWallet = async () => {
    ...
    if (window.ethereum) {
      try {
        ...
        // Bind data for both Spanish Fiesta and Pro Pool Packages
        await BindSpanishFiestaPackData();
        await BindProPoolData();
        ...
        const achievedCapping = web3.utils.fromWei(userBonusData[1], "ether");
        ...
        let CapingPercentage =
          (parseFloat(achievedCapping) / parseFloat(totalCapping)) * 100;
  ...
  // For purchased Pro Pool Packages
  const GetUserProPoolSummary = async (proPoolId) => {
    try {
      if (!contractInstance || !account) {
        console.error("Contract instance or account not initialized.");
        return;
      }

      const res = await contractInstance.methods
        .pro_pool_history(account, proPoolId)
        .call();
      ...
      if (res[0] === "true" || res[0] === true) {
        ...
        const poolIndex = parseInt(proPoolId);

        // Set common values for all pro pool ids
        updatedData.purchasePoolFlag[poolIndex] = 1;
  ...
  const GetUserSpanishFiestaPackSummary = async (spanishFiestaPackPoolId) => {
    try {
      if (!contractInstance || !account) {
        console.error("Contract instance or account not initialized.");
        return;
      ...
      // Fetch Spanish Fiesta pool history from the contract
      const res = await contractInstance.methods
        .spanish_fiesta_pool_history(account, spanishFiestaPackPoolId)
        .call();
      
      const updatedData = { ...spanishFiestaPackData };

      // Check if the pool was purchased
      if (res[0] === "true" || res[0] === true) {
        const remainingMonth = 8 - parseInt(res[3]) || 0;
        const remainingDays = 30 - parseInt(res[4]) || 0;
        const poolIndex = parseInt(spanishFiestaPackPoolId);

        // Set common values for all Spanish Fiesta pack ids
        ...
        // Handle the not purchased state
        if (
          spanishFiestaPackPoolId === 0 &&
          parseInt(document.getElementById("noofdirect").value) < 3
        ) {
          updatedData.purchasePoolFlag[0] = 0;
        } else if (
          spanishFiestaPackPoolId >= 1 &&
          updatedData.purchasePoolFlag[spanishFiestaPackPoolId] === 0
        ) {
          updatedData.purchasePoolFlag[spanishFiestaPackPoolId] = 0;
        }
      }
      ...
      console.error("Error fetching Spanish Fiesta pool history:", error);
    }
  };
